...
export default function Home() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await TaskService.getAllTasks(currentPage-1, pageSize , user.token)
    ...
    const exportPdf = () => {
      TaskService.getAllTasks(0, totalElements, user.token).then((res) =>{
    ...
    const handleSearchChange = (event: any) => {
      ...
      TaskService
        .searchTasks(searchType , searchValue , user.token)
        .then((response) => {
          if (response.status === 200) {
            setLoading(false)
